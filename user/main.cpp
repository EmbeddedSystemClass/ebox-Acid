...
//////////////////////
uint16_t voltage_to_temperature(float voltage,float ref,const float *table)
{
...
//*********串口发送输出结果给主控单片机和接收主控单片机参数配置指令*********//
        if(millis() - last_updata_time >= 200) //发送485更新速率时间为100ms每次
        {
            last_updata_time = millis();
          _485_tx_mode();
...
//            uart1.printf("%04d\t%0.4f\n",val_ch2,detector_temperature_ch2);               
           ...
           uint16_t temp1 = voltage_to_temperature(K_voltage_ch3,LM35_voltage_ch4,k_600_table);
           uint16_t temp2 = voltage_to_temperature(S_voltage_ch2,LM35_voltage_ch4,s_300_table);
           temp2 = temp2 + 3;//修正温度校准误差
           uart1.printf("temp1:%d\r\ntemp2:%d",temp1,temp2);
 //485发送数据方式          
