...
/*
        对电流检测进行两个函数分段处理，提高精度，需要改进电路降低噪声。就可以去除本部分
*/
        if(val_ch1 >= 159 )
        {
            I_voltage_ch1 = ch1_off.val + ch1_ratio.val*val_ch1;//对电流检测进行分段处理
        }
...
//          uart1.printf("val_ch1 = 0x%04x\r\nval_ch2 = 0x%04x\r\nval_ch3 = 0x%04x\r\n val_ch4 = 0x%04x\r\n",I_voltage_ch1,val_ch2,val_ch3,val_ch4);
 ...
 //查表得出热电偶温度值         
         ...
         //uart1.printf("temp1:%d\ttemp2:%d\tvoltage:%0.2f\r\n",temp1,temp2,K_voltage_ch3);//校准温度用来观看数据使用

/***************************调试程序********************************************************/
/* 2018.6.11串口测试程序，去除备注既可以调试模拟板至上位机观察原始数据*/
/****************************调试程序******************************************************/
...
void adc_read_ch(uint8_t ch)
{
    LED1.toggle();//ADC采甲刺指示灯，采样率由模拟开关切换速率决定
